...
>
    <x-slot:breadcrumbs>
        <x-tallui-breadcrumb :links="[
            ['label' => 'HR', 'href' => route('hr.entities.employees.index')],
    ...
    <x-slot:actions>
        ...
        <x-tallui-button
            ...
            icon="o-plus"
            :link="route('hr.entities.' . $entity . '.create')"
                        ...
                        @foreach ($columns as $column)
                            <td class="pl-4 first:pl-5 text-sm">
                                ...
                                @elseif (strlen((string) $value) > 60)
                                    <span title="{{ $value }}">{{ substr((string) $value, 0, 57) }}...</span>
                                @else
                                    {{ $value ?? '-' }}
                                @endif
                            </td>
                        ...
                        <td class="pr-5">
                            <div class="flex justify-end gap-1">
                                <x-tallui-button
                                    ...
                                    :link="route('hr.entities.' . $entity . '.edit', ['recordId' => $record->getKey()])"
