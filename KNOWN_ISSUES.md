...
- `vendor/bin/phpstan analyse` (level `max`) reports **211 errors**, despite `phpstan-baseline.neon` already baselining 24 error signatures — these 211 are unbaselined/live. Two things stand out as more than generic "missing generic type" noise:
  ...
  - **`src/Support/Zkteco/RatsZktecoClient.php`** (lines 25, 30, 35, 40) — `Instantiated class \Rats\Zkteco\Lib\ZKTeco not found` and subsequent "call to undefined method" on the resulting object — the `rats/zkteco` dependency doesn't appear to be present/autoloadable in this analysis context.
