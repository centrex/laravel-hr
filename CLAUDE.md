...
`laravel-package-laravel-hr` — Template/scaffold for creating new Laravel packages in this monorepo.

Namespace: `Centrex\Hr\` (replace with actual vendor/package name)

This is the reference template. When creating a new package, copy this directory and do a find-and-replace on `Centrex`, `Hr`, `laravel-hr`, and `package-description`.
...
1. Replace all `Centrex\Hr` namespace references
2. Update `composer.json` name, description, and autoload namespace
3. Rename `Hr.php` and `HrServiceProvider.php`
